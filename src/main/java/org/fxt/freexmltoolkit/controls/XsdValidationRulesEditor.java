...
/**
 * Professional XSD Validation Rules Editor with comprehensive constraint management
 * <p>
 * Features:
 * - Pattern (RegEx) Editor with Live Preview
 * - Enumeration Values Manager
 * - Range Constraints (min/max Inclusive/Exclusive)
 * - Length Constraints (length, minLength, maxLength)
 * - Decimal Constraints (totalDigits, fractionDigits)
 * - Whitespace Handling
 * - Custom Facet Support
 * - Live Validation Preview
 * - Import/Export Validation Rules
 */
public class XsdValidationRulesEditor extends Dialog<ValidationRulesResult> {

    private static final Logger logger = LogManager.getLogger(XsdValidationRulesEditor.class);
    ...
    public XsdValidationRulesEditor(XsdNodeInfo targetNode) {
        ...
        logger.info("XsdValidationRulesEditor initialized for node: {}", targetNode.name());
    }
    ...
    private void initializeDialog() {
        ...
        getDialogPane().getStylesheets().add(
                getClass().getResource("/css/xsd-validation-editor.css").toExternalForm()
        );

        // Configure result converter
        setResultConverter(buttonType -> {
            if (buttonType == ButtonType.OK) {
                return collectValidationRules();
            }
            return null;
        });
    ...
    private void createPatternTab() {
        ...
        patternTab.setContent(new ScrollPane(content));
    ...
    private void createEnumerationTab() {
        ...
        // Enable row selection and refresh table on edits
        enumerationTable.getSelectionModel().setSelectionMode(SelectionMode.SINGLE);
        enumerationTable.setRowFactory(tv -> {
            TableRow<EnumerationValue> row = new TableRow<>();
            return row;
        });
    ...
    private void loadCurrentConstraints() {
        logger.debug("Loading current constraints for node: {}", targetNode.name());

        try {
            // Load existing enumeration values from XSD DOM
            loadExistingEnumerationValues();

            // TODO: Load other constraint types (pattern, range, etc.)

        } catch (Exception e) {
            logger.error("Error loading current constraints for node: " + targetNode.name(), e);
        }
    }

    private void loadExistingEnumerationValues() {
        // This method will extract real enumeration values from the XSD DOM
        // For now, we start with an empty list since we removed sample data
        enumerationData.clear();

        // TODO: Implement actual XSD DOM parsing to extract enumeration facets
        // Example logic:
        // 1. Find the element in the DOM using targetNode.xpath()
        // 2. Look for simpleType/restriction/enumeration elements
        // 3. Extract the 'value' attributes
        // 4. Add them to enumerationData

        logger.debug("Loaded {} existing enumeration values", enumerationData.size());
    }

    private void setupEventHandlers() {
        // Additional event handlers can be added here
        logger.debug("Event handlers configured for XsdValidationRulesEditor");
    }

    private ValidationRulesResult collectValidationRules() {
        ValidationRulesResult result = new ValidationRulesResult();

        // Collect pattern
        if (!patternField.getText().trim().isEmpty()) {
            result.setPattern(patternField.getText().trim());
        }

        // Collect enumeration values
        List<String> enumValues = enumerationData.stream()
        ...
        if (!minInclusiveField.getText().trim().isEmpty()) {
            result.setMinInclusive(minInclusiveField.getText().trim());
        }
        if (!maxInclusiveField.getText().trim().isEmpty()) {
            result.setMaxInclusive(maxInclusiveField.getText().trim());
        }
        if (!minExclusiveField.getText().trim().isEmpty()) {
            result.setMinExclusive(minExclusiveField.getText().trim());
        }
        if (!maxExclusiveField.getText().trim().isEmpty()) {
            result.setMaxExclusive(maxExclusiveField.getText().trim());
        }

        // Collect length constraints
        if (!lengthField.getText().trim().isEmpty()) {
            result.setLength(lengthField.getText().trim());
        }
        if (!minLengthField.getText().trim().isEmpty()) {
            result.setMinLength(minLengthField.getText().trim());
        }
        if (!maxLengthField.getText().trim().isEmpty()) {
            result.setMaxLength(maxLengthField.getText().trim());
        }

        // Collect decimal constraints
        if (!totalDigitsField.getText().trim().isEmpty()) {
            result.setTotalDigits(totalDigitsField.getText().trim());
        }
        if (!fractionDigitsField.getText().trim().isEmpty()) {
            result.setFractionDigits(fractionDigitsField.getText().trim());
        }

        // Collect whitespace action
        result.setWhitespaceAction(whitespaceComboBox.getValue());

        // Collect custom facets
        List<CustomFacet> customFacets = customFacetsData.stream()
                .filter(f -> !f.getName().trim().isEmpty() && !f.getValue().trim().isEmpty())
                .toList();
        if (!customFacets.isEmpty()) {
            result.setCustomFacets(customFacets);
        }

        logger.info("Collected validation rules for node: {}", targetNode.name());
        return result;
    }
