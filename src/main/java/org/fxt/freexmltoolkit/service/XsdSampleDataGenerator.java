...
public class XsdSampleDataGenerator {
    ...
    private String generateFromPatternWithLengthRestrictions(RestrictionInfo restriction, String elementName) {
        ...
        // Smart pattern recognition for known types BEFORE falling back to Generex
        String smartValue = generateSmartPatternValue(patternValue, restriction);
        if (smartValue != null) {
            logger.debug("Generated smart value for pattern '{}' element '{}': {}", patternValue, elementName, smartValue);
            return smartValue;
        }
    ...
    // =========================================================================
    // Smart Pattern Recognition for Known Types
    // =========================================================================

    /**
     * Recognizes well-known patterns and generates appropriate values.
     * This handles patterns that Generex struggles with (complex groupings, email, etc.)
     *
     * @param pattern     The XSD pattern
     * @param restriction The restriction info for additional context
     * @return Generated value or null to fall through to Generex
     */
    private String generateSmartPatternValue(String pattern, RestrictionInfo restriction) {
        // LEI pattern: ([0-9a-zA-Z]{18}[0-9]{2}) - exactly 20 chars: 18 alphanumeric + 2 digits
        if (pattern.contains("[0-9a-zA-Z]{18}") || pattern.contains("[a-zA-Z0-9]{18}")) {
            return generateLEI();
        }

        // Email pattern: contains @ and typical email structure
        if (pattern.contains("@") && (pattern.contains("[a-zA-Z0-9]") || pattern.contains("[A-Za-z0-9]"))) {
            return generateEmail();
        }

        // ISIN pattern: [A-Z]{2}[A-Z0-9]{9}[0-9] or similar 12-char alphanumeric
        if ((pattern.contains("[A-Z]{2}") && pattern.contains("[0-9]") &&
             (getLengthFromRestriction(restriction) == 12 || pattern.contains("{9}"))) ||
            pattern.equals("[A-Z0-9]{12}")) {
            return generateISIN();
        }

        // ISO Currency Code: [A-Z]{3}
        if (pattern.equals("[A-Z]{3}")) {
            return generateCurrencyCode();
        }

        // BIC/SWIFT pattern: [A-Z]{6}[A-Z0-9]{2}([A-Z0-9]{3})?
        if (pattern.contains("[A-Z]{6}") && pattern.contains("[A-Z0-9]{2}")) {
            return generateBIC();
        }

        // Phone number patterns: typically contain digits with optional +, -, spaces
        if (pattern.contains("[0-9]") && (pattern.contains("\\+") || pattern.contains("\\-")) &&
            !pattern.contains("@") && !pattern.contains("[A-Z]")) {
            return generatePhoneNumber();
        }

        return null; // Fall through to Generex
    }

    /**
     * Gets the expected length from restriction facets.
     */
    private int getLengthFromRestriction(RestrictionInfo restriction) {
        if (restriction == null) return -1;
        Integer exact = getIntFacet(restriction.facets(), "length");
        if (exact != null) return exact;
        Integer min = getIntFacet(restriction.facets(), "minLength");
        Integer max = getIntFacet(restriction.facets(), "maxLength");
        if (min != null && max != null && min.equals(max)) return min;
        return -1;
    }

    /**
     * Generates a valid LEI (Legal Entity Identifier).
     * Format: 18 alphanumeric characters + 2 check digits = 20 characters total
     */
    private String generateLEI() {
        StringBuilder sb = new StringBuilder();
        String alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

        // First 18 characters: alphanumeric
        for (int i = 0; i < 18; i++) {
            sb.append(alphanumeric.charAt(ThreadLocalRandom.current().nextInt(alphanumeric.length())));
        }

        // Last 2 characters: digits (check digits)
    ...
    private String generateEmail() {
        String[] users = {"user", "info", "contact", "support", "admin", "test", "sample"};
    ...
    /**
     * Generates a valid ISIN (International Securities Identification Number).
     * Format: 2-letter country code + 9 alphanumeric + 1 check digit = 12 characters
     */
    private String generateISIN() {
        StringBuilder sb = new StringBuilder();
        String[] countryCodes = {"US", "DE", "GB", "FR", "JP", "CH", "CA", "AU"};
        String alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";

        // Country code (2 letters)
        sb.append(countryCodes[ThreadLocalRandom.current().nextInt(countryCodes.length)]);
    ...
    private String generateCurrencyCode() {
        String[] codes = {"USD", "EUR", "GBP", "JPY", "CHF", "CAD", "AUD", "CNY", "HKD", "SGD"};
        return codes[ThreadLocalRandom.current().nextInt(codes.length)];
    }

    /**
     * Generates a valid BIC/SWIFT code.
    ...
    private String generateBIC() {
        ...
        String alphanumeric = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
        String[] countryCodes = {"US", "DE", "GB", "FR", "CH", "NL", "BE", "AT"};
        ...
        // Optionally add branch code (3 alphanumeric) - 50% chance
    ...
    private String generatePhoneNumber() {
        return "+1-555-" + String.format("%03d", ThreadLocalRandom.current().nextInt(1000)) +
