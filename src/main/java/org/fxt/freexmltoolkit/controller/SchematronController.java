...
public class SchematronController {
    ...
    /**
     * Open the official Schematron website in the default browser
     */
    @FXML
    private void openSchematronWebsite() {
        try {
            java.awt.Desktop.getDesktop().browse(new java.net.URI("https://schematron.com/"));
            logger.info("Opened Schematron website");
        } catch (Exception e) {
            logger.error("Failed to open Schematron website", e);
            showAlert(Alert.AlertType.ERROR, "Error", "Could not open browser: " + e.getMessage());
        }
    }

    /**
     * Open the ISO Schematron specification in the default browser
     */
    @FXML
    private void openISOSpec() {
        try {
            java.awt.Desktop.getDesktop().browse(new java.net.URI("https://www.iso.org/standard/74515.html"));
            logger.info("Opened ISO Schematron specification");
        } catch (Exception e) {
            logger.error("Failed to open ISO specification", e);
            showAlert(Alert.AlertType.ERROR, "Error", "Could not open browser: " + e.getMessage());
        }
    }

    /**
     * Open XPath 2.0 reference in the default browser
     */
    @FXML
    private void openXPathReference() {
        try {
            java.awt.Desktop.getDesktop().browse(new java.net.URI("https://www.w3.org/TR/xpath20/"));
            logger.info("Opened XPath 2.0 reference");
        } catch (Exception e) {
            logger.error("Failed to open XPath reference", e);
            showAlert(Alert.AlertType.ERROR, "Error", "Could not open browser: " + e.getMessage());
        }
    }
}